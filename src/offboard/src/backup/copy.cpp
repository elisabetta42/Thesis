	...
	//switch (str2int(msg->data.c_str())) {
	  geometry_msgs::Point pr2_position;
        ...
        if(string(msg->data.c_str()).compare("right")== 0){
	     ...
	     pr2_position.x = getmodelstate.response.pose.position.x+1;
    	     pr2_position.y  = getmodelstate.response.pose.position.y;
             pr2_position.z  = getmodelstate.response.pose.position.z;
            }
	 ...
	 gazebo_msgs::ModelState pr2_modelstate;
    	 pr2_modelstate.model_name = (std::string) "iris_1";
         pr2_modelstate.pose = pose;
