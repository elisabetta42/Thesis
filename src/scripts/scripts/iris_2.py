...
# Initial conditions
	...
	result=rules.flocking(uav_name, blockListDict)
	self.vres[0]=result[0] #+ self.x_off
	self.vres[1]=result[1] #+ self.y_off
...
if __name__ == '__main__':
	 ...
	 drone_list=Tutorial()
	 arm_pub=rospy.Publisher("gazebo/iris_2/go_to_destination", TwistStamped,queue_size=1000)	 
   	
	 while True:	
		vres=drone_list.swarm("iris_2", drone_list.blockListDict)		
		#goal = PoseStamped()
		goal=TwistStamped()
	        alt=rules.getaltitude("iris_2")
		goal=TwistStamped()
		goal.header.frame_id = "/base_link"
		goal.header.stamp = rospy.Time.now()
		goal.twist.linear.x =vres[0] #+ drone_list.x_off
		goal.twist.linear.y =vres[1] #+ drone_list.y_off              
		goal.twist.linear.z = 2 - alt
